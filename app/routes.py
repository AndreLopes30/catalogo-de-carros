...
def catalogo():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO carros (modelo, ano, preco, imagem) VALUES (?, ?, ?, ?)", 
    ...
    carros = [{'id': r[0], 'modelo': r[1], 'ano': r[2], 'preco': formatar_preco(r[3]), 'imagem': r[4]} for r in rows]
    conn.close()
    return render_template('catalogo.html', carros=carros, pagina=pagina, total_paginas=total_paginas, busca=busca)
...
def infos(id):
    ...
    row = cursor.fetchone()
    conn.close()
    if not row: return "Carro não encontrado", 404
    carro = {'id': row[0], 'modelo': row[1], 'ano': row[2], 'preco': formatar_preco(row[3]), 'imagem': row[4]}
...
def editar(id):
    ...
    if request.method == 'POST':
        modelo, ano, imagem = request.form['modelo'], int(request.form['ano']), request.form['imagem']
        try:
            preco = normalizar_preco(request.form['preco'])
        except ValueError:
            conn.close()
        ...
        if modelo == '' or ano < 1900 or ano > datetime.now().year + 1 or preco < 0 or not validar_url_imagem(imagem):
            conn.close()
            return redirect(f'/carro/{id}/editar')
        
        cursor.execute("UPDATE carros SET modelo=?, ano=?, preco=?, imagem=? WHERE id=?", (modelo, ano, preco, imagem, id))
        conn.commit()
        conn.close()
